...
public class UsersControllerTest {
    ...
    public void shouldGetAllUsers() throws Exception {
        ...
        mockMvc.perform(get("/api/v2/get-clients"))
    ...
    public void shouldGetByIdUsers() throws Exception {
        ...
        ClientInfo client = new ClientInfo(id, "test", "phone", birthday, true);
        when(clientServiceImpl.getClientById(id)).thenReturn(client);
        mockMvc.perform(get("/api/v2/get-client/{id}", id))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is(id)))
                .andExpect(jsonPath("$.fullName", is("test")))
                .andExpect(jsonPath("$.phone", is("phone")))
                .andExpect(jsonPath("$.birthday[0]", is(1999)))
                .andExpect(jsonPath("$.birthday[1]", is(11)))
                .andExpect(jsonPath("$.birthday[2]", is(4)))
